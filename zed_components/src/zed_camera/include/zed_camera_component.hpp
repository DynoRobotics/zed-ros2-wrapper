...
class ZedCamera : public rclcpp::Node {
 ...
 protected:
  ...
  // ----> Callbacks
  void callback_saveAreaMap(const std_msgs::msg::String::SharedPtr msg);
  ...
  void publishOdom(tf2::Transform& odom2baseTransf, sl::Pose& slPose,
                   rclcpp::Time t);
  void publishPose();
  ...
  std::string parseRoiPoly(const std::vector<std::vector<float>>& in_poly,
 ...
 private:
  ...
  int mStreamPort =
      30000;  // The port to be used to connect to a local streaming server
  ...
  // ----> Subscribers
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr mSaveAreaMapSub;
  ...
  sl::FUSION_ERROR_CODE mFusionStatus =
      sl::FUSION_ERROR_CODE::MODULE_NOT_ENABLED;
